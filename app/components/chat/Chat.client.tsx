...
import { chatStore, setChatModel } from '~/lib/stores/chat';
...
export function Chat() {
  ...
  const { ready, initialMessages, storeMessageHistory, sessionId } = useChatHistory();
...
export const ChatImpl = memo(({ initialMessages, storeMessageHistory, sessionId }: ChatProps) => {
  useShortcuts();
  ...
  const { showChat, model } = useStore(chatStore);
    ...
    <BaseChat
      ...
      enhancePrompt={() => {
        enhancePrompt(
          ...
          const message = error instanceof Error ? error.message : 'Enhancer request failed';
          toast.error(message);
        });
      }}
      selectedModel={model}
      onModelChange={(value) => {
        setChatModel(value as LlmModelId);
      }}
    />
  );
});
